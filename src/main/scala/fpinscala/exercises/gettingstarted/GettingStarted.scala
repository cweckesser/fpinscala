...
object MyProgram:
  ...
  def fib(n: Int): Int = {
    @annotation.tailrec
    def loop(n: Int, index: Int, secondPrevious: Int, previous: Int): Int = {
      val newPrevious = secondPrevious + previous
      if (n == index) newPrevious
      else loop(n, index + 1, previous, newPrevious)
    }
